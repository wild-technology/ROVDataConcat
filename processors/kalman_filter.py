	...
	try:
		...
		if df["Timestamp"].duplicated().any():
			...
			if "event_value" in df.columns:
				df = df.sort_values("event_value", na_position="first").drop_duplicates(subset=["Timestamp"],
																						keep="first")
			else:
...
if __name__ == "__main__":
	# If run directly, use the current directory as raw data directory
	# and a subdirectory "processed" for output.
	raw_directory = Path.cwd()
