...
def process_data(raw_dir, processed_dir):
    ...
    print(f"Merged dataframe shape before duplicate removal: {merged_df.shape}")

    # Run additional duplicate timestamp check with priority for non-null event_value
    ...
    # Optionally, print per-Timestamp duplicate counts (sample output)
    if duplicate_counts:
...
if __name__ == "__main__":
    raw_directory = input("Enter the raw directory for processing: ").strip()
    processed_directory = input("Enter the processed directory for output: ").strip()
